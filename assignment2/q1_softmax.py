...
def softmax(x):
    ...
    ### YOUR CODE HERE
    if len(x.shape)>1:
        mid1 = x - tf.reduce_max(x,axis=1,keepdims=True)
        mid2 = tf.exp(mid1)
        mid3 = tf.log(tf.reduce_sum(mid2,axis=1,keepdims=True))
        out = tf.exp(mid1-mid3)
    else:
        mid1 = x - tf.reduce_max(x,keepdims=True)
        mid2 = tf.exp(mid1)
        mid3 = tf.log(tf.reduce_sum(mid2,keepdims=True))
        out = tf.exp(mid1-mid3)
    ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
    ...
    out = -tf.reduce_sum(y*tf.log(yhat)+(1-y)*tf.log(1-yhat))*0.5
