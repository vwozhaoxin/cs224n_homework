...
class Config:
    ...
    def __init__(self, output_path=None):
        if output_path:
            ...
            self.output_path = "results/window/{:%Y%m%d_%H%M}/".format(datetime.now())
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        if l==1:
#            print(l,sentence,labels)
            new_sentence = start +sentence[0]+end
            windowed_data.append((new_sentence,labels[0]))
        else:
            for i in range(l):          
                if window_size==1:
                    if i ==0 :
#                for lengthin range(window_size):
                        new_sentence = start* window_size+sentence[0]+sentence[1]
                    elif i==l-window_size:
                        new_sentence = sentence[-2]+sentence[-1]+end*window_size
                    else:
                        new_sentence = sentence[i-1]+sentence[i]+sentence[i+1]
#            print(new_sentence,labels)
                    windowed_data.append((new_sentence,labels[i]))
...
#                     self.labels_placeholder:labels_batch,
    ...
    def predict_on_batch(self, sess, inputs_batch):
        ...
        """
#        logger.info("preduct_on_batch_input_type",type(inputs_batch),inputs_batch.shape)
        feed = self.create_feed_dict(inputs_batch)
        predictions = sess.run(tf.argmax(self.pred, axis=1), feed_dict=feed)
        return predictions

    def train_on_batch(self, sess, inputs_batch, labels_batch):
#        logging.info("input_shape %s",inputs_batch.shape)
...
if __name__ == "__main__":
    ...
    command_parser.add_argument('-v', '--vocab', type=argparse.FileType('r'), default="data/vocab.txt", help="Path to vocabulary file")
    command_parser.add_argument('-vv', '--vectors', type=argparse.FileType('r'), default="data/wordVectors.txt", help="Path to word vectors file")
    command_parser.add_argument('-o', '--output', type=argparse.FileType('wb'), default=sys.stdout, help="Training data")
